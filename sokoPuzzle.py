...
class SokoPuzzle:
    ...
    def isDead(self, deadlock_map):

        # Retrieve all the dead locks
        D_indices_x, D_indices_y = np.where(np.array(deadlock_map) == 'D')

        # Check if the the dead locks contain blocks
        for ind_x, ind_y in zip(D_indices_x, D_indices_y):
            if self.robot_block[ind_x][ind_y] == 'B':
                return True
        return False
